/**
 * @file cece_performance_diagnostics.hpp
 * @brief Performance timing and memory diagnostics for CECE
 */

#ifndef CECE_PERFORMANCE_DIAGNOSTICS_HPP
#define CECE_PERFORMANCE_DIAGNOSTICS_HPP
...
class PerformanceDiagnostics {
   public:
    ...
    void PrintMemorySummary() const {
        std::cout << "\n=== CECE Memory Usage Summary ===" << std::endl;
...
#define CECE_TIMER_START(name) cece::PerformanceDiagnostics::GetInstance().StartTimer(name)

#define CECE_TIMER_STOP(name) cece::PerformanceDiagnostics::GetInstance().StopTimer(name)
