...
double vertical_dust_flux(double u, double u_t, double rho_air, double f_erod, double k_gamma) {
    ...
    return C_d * f_erod * k_gamma * rho_air * ((u * u - u_t * u_t) / u_st) * std::pow(u / u_t, C_a * f_ust);
}

double drag_partition(double z0, double z0s) {
    constexpr double z0_max = 5.0e-4;
    if (z0 <= z0s || z0 >= z0_max) return 1.0;
    return 1.0 - std::log(z0 / z0s) / std::log(0.7 * std::pow(122.55 / z0s, 0.8));
}
   ...
   public:
    ...
    void fill_k14_import_state(CeceImportState& import_state, int nx, int ny) {
        ...
        // soil_texture: integer values 1-12 stored as double
        {
            ...
            for (int j = 0; j < ny; ++j)
                for (int i = 0; i < nx; ++i) h(i, j, 0) = static_cast<double>(rand_int(1, 12));
        ...
        // vegetation_type: 7 (open shrublands) or 16 (barren) for emitting cells
        {
            ...
            for (int j = 0; j < ny; ++j)
                for (int i = 0; i < nx; ++i) h(i, j, 0) = (rand_uniform(0.0, 1.0) < 0.5) ? 7.0 : 16.0;
...
TEST_F(K14PropertyTest, Property5_KokVerticalDustFlux) {
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        double f_ust = (u_st - u_st0) / u_st0;
        double C_d = C_d0 * std::exp(-C_e * f_ust);
        double expected = C_d * f_erod * k_gamma * rho_air * ((u * u - u_t * u_t) / u_st) * std::pow(u / u_t, C_a * f_ust);

        EXPECT_NEAR(result, expected, std::abs(expected) * 1e-14 + 1e-20)
            << "Failed at iter=" << iter << " u=" << u << " u_t=" << u_t << " rho_air=" << rho_air;
...
TEST_F(K14PropertyTest, Property6_MacKinnonDragPartition) {
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        double expected = 1.0 - std::log(z0 / z0s) / std::log(0.7 * std::pow(122.55 / z0s, 0.8));

        EXPECT_NEAR(result, expected, std::abs(expected) * 1e-14 + 1e-14) << "Failed at iter=" << iter << " z0=" << z0 << " z0s=" << z0s;
    }
    ...
    for (int iter = 0; iter < 50; ++iter) {
        ...
        double result = k14_ref::drag_partition(z0, z0s);
        EXPECT_DOUBLE_EQ(result, 1.0) << "Should return 1.0 when z0 <= z0s, iter=" << iter;
    }
    ...
    for (int iter = 0; iter < 50; ++iter) {
        ...
        double result = k14_ref::drag_partition(z0, z0s);
        EXPECT_DOUBLE_EQ(result, 1.0) << "Should return 1.0 when z0 >= z0_max, iter=" << iter;
    }
}
...
TEST_F(K14PropertyTest, Property7_SmoothRoughnessLookup) {
    ...
    for (int texture = 1; texture <= 12; ++texture) {
        ...
        EXPECT_DOUBLE_EQ(result, expected) << "Mismatch for texture=" << texture;
    }
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        if (texture >= 1 && texture <= 12) {
            double expected = Dc_soil[texture - 1] / 30.0;
            EXPECT_DOUBLE_EQ(result, expected) << "Mismatch for valid texture=" << texture;
        } else {
            EXPECT_DOUBLE_EQ(result, 125e-6 / 30.0) << "Invalid texture=" << texture << " should return default";
        }
    }
}
...
TEST_F(K14PropertyTest, Property8_LaurentErodibility) {
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        if (texture == 15) {
            // Bedrock: always 0
            EXPECT_DOUBLE_EQ(result, 0.0) << "Bedrock (texture==15) should return 0.0, iter=" << iter;
        } else if (z0 > 3.0e-5 && z0 < 5.0e-4) {
            // Laurent formula range
            double expected = 0.7304 - 0.0804 * std::log10(100.0 * z0);
            EXPECT_NEAR(result, expected, std::abs(expected) * 1e-14 + 1e-14) << "Failed at iter=" << iter << " z0=" << z0 << " texture=" << texture;
        } else {
            // Outside range: 1.0
            EXPECT_DOUBLE_EQ(result, 1.0) << "Outside range should return 1.0, iter=" << iter << " z0=" << z0 << " texture=" << texture;
        }
    }
}
...
TEST_F(K14PropertyTest, Property10_ZeroEmissionInvariant) {
    ...
    for (int iter = 0; iter < 50; ++iter) {
        ...
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < 2; ++i) f_land_h(i, j, 0) = 0.0;

        // Rows 2-3: roughness above z0_max (5.0e-4)
        for (int j = 0; j < ny; ++j)
            for (int i = 2; i < 4; ++i) z0_h(i, j, 0) = rand_uniform(5.0e-4, 1.0e-2);
        ...
        for (int j = 0; j < ny; ++j)
            for (int i = 4; i < 6; ++i) ustar_h(i, j, 0) = rand_uniform(0.0, 0.001);
