...
class AutoRegistrationTestScheme : public BasePhysicsScheme {
 public:
  ...
  bool WasFinalized() const { return finalized_; }
  double GetTestParam() const { return test_param_; }
...
class DynamicTestScheme : public BasePhysicsScheme {
 public:
  explicit DynamicTestScheme(const std::string& name = "dynamic_test")
      : scheme_name_(name) {}
  ...
  bool WasRunCalled() const { return run_called_; }
  bool WasFinalized() const { return finalized_; }
  const std::string& GetSchemeName() const { return scheme_name_; }
...
TEST_F(SchemeAutoRegistrationTest, BasicSchemeDiscovery) {
  ...
  if (test_scheme != nullptr) {
    EXPECT_TRUE(test_scheme->WasRunCalled())
        << "Scheme should execute Run() method";
  }
  ...
  if (test_scheme != nullptr) {
    EXPECT_TRUE(test_scheme->WasFinalized())
        << "Scheme should finalize after Finalize() call";
  }
}
...
TEST_F(SchemeAutoRegistrationTest, MultipleIndependentInstances) {
  ...
  EXPECT_NE(scheme1.get(), scheme2.get())
...
TEST_F(SchemeAutoRegistrationTest, DynamicSchemeRegistration) {
  ...
  ASSERT_NE(scheme, nullptr)
      << "Dynamically registered scheme should be discoverable";
...
TEST_F(SchemeAutoRegistrationTest, FactoryConsistency) {
  ...
  for (int i = 0; i < num_iterations; ++i) {
    for (int j = i + 1; j < num_iterations; ++j) {
      EXPECT_NE(schemes[i].get(), schemes[j].get())
