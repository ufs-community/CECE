...
extern "C" {
void ACES_Initialize(ESMC_GridComp comp, ESMC_State importState,
                     ESMC_State exportState, ESMC_Clock* clock, int* rc);
void ACES_Run(ESMC_GridComp comp, ESMC_State importState,
              ESMC_State exportState, ESMC_Clock* clock, int* rc);
void ACES_Finalize(ESMC_GridComp comp, ESMC_State importState,
                   ESMC_State exportState, ESMC_Clock* clock, int* rc);
}
...
int main(int argc, char** argv) {
  ...
  auto createField = [&](ESMC_State state, const char* name) {
    ESMC_Field field = ESMC_FieldCreateGridTypeKind(
        grid, ESMC_TYPEKIND_R8, ESMC_STAGGERLOC_CENTER, NULL, NULL, NULL, name,
        &rc);
  ...
  ESMC_Calendar cal =
      ESMC_CalendarCreate("Gregorian", ESMC_CALKIND_GREGORIAN, &rc);
  ...
  ESMC_Clock clock =
      ESMC_ClockCreate("SimulationClock", timeStep, startTime, stopTime, &rc);
  ...
  for (int i = 0; i < nx * ny * nz; ++i) {
    ...
    if (std::abs(total_ptr[i] - expected) > 1e-6) {
        std::cerr << "Mismatch at " << i << " (x=" << x << "): expected " << expected << ", got " << total_ptr[i] << std::endl;
        success = false;
        break;
    }
  }
