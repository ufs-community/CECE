...
class MockFieldResolver : public FieldResolver {
 ...
 public:
  ...
  UnmanagedHostView3D ResolveImport(const std::string& name, int /*nx*/,
                                    int /*ny*/, int /*nz*/) override {
    if (fields.count(name)) return fields[name].view_host();
    return UnmanagedHostView3D();
  }

  UnmanagedHostView3D ResolveExport(const std::string& name, int /*nx*/,
                                    int /*ny*/, int /*nz*/) override {
    if (fields.count(name)) return fields[name].view_host();
    return UnmanagedHostView3D();
  }
  ...
  ResolveExportDevice(const std::string& name, int /*nx*/, int /*ny*/, int /*nz*/) override {
    if (fields.count(name)) return fields[name].view_device();
