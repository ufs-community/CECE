...
static std::vector<double> ReadNetCDFVariable(int ncid, int varid, int nx, int ny, int nz,
    ...
    // Layout check: Kokkos::LayoutLeft(x, y, z) means:
    // index = i + j*nx + k*nx*ny
    // NetCDF (time, z, y, x) in row-major means:
    // data[k*ny*nx + j*nx + i]

    // Convert from row-major (z, y, x) to LayoutLeft (x, y, z)
    std::vector<double> layout_left_data(nx * ny * nz);
    for (int k = 0; k < nz; ++k) {
        for (int j = 0; j < ny; ++j) {
            for (int i = 0; i < nx; ++i) {
                int rm_idx = k * (ny * nx) + j * nx + i;
                int ll_idx = i + j * nx + k * nx * ny;
