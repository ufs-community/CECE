...
std::vector<double> Flatten(DualView3D& dv) {
    ...
    for (size_t i = 0; i < h.extent(0); ++i)
        for (size_t j = 0; j < h.extent(1); ++j)
            for (size_t k = 0; k < h.extent(2); ++k) out.push_back(h(i, j, k));
    return out;
}
...
TEST(IngestorReinitRoundtripProperty, ReinitProducesIdenticalValues) {
    ...
    ASSERT_EQ(v1.size(), v2.size()) << "Property 9: reinit must produce same-size field";

    for (size_t i = 0; i < v1.size(); ++i) {
        EXPECT_DOUBLE_EQ(v1[i], v2[i])
            << "Property 9: value mismatch at index " << i << " after reinit roundtrip";
    }
