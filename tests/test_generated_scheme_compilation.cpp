 ...
 private:
  ...
  std::string GenerateSchemeName() {
    static const std::vector<std::string> prefixes = {
        "custom", "test", "example", "generated", "auto"};
    static const std::vector<std::string> suffixes = {
        "emission", "process", "scheme", "kernel", "compute"};
  ...
  std::string GenerateUnits() {
    static const std::vector<std::string> units = {
        "K",           "m/s",        "kg/m3",      "kg/m2/s",    "Pa",
        "dimensionless", "W/m2",     "mol/mol",    "ppm",        "ppb"};
...
class GeneratedSchemeCompilationTest : public ::testing::Test {
 protected:
  ...
  bool CompileGeneratedScheme(const std::string& scheme_name) {
    ...
    std::string compile_cmd = "g++ -std=c++20 -I. -c " + impl_file +
...
TEST_F(GeneratedSchemeCompilationTest, GeneratedSchemesHaveRequiredMethods) {
  ...
  // Run scheme generator
  ASSERT_TRUE(RunSchemeGenerator(config_file))
      << "Scheme generator failed";
...
TEST_F(GeneratedSchemeCompilationTest, VariousFieldCombinations) {
  ...
  for (const auto& [num_imports, num_exports, num_diagnostics] : combinations) {
    ...
    ss << "scheme:\n";
    ss << "  name: TestScheme\n";
    ss << "  description: \"Test scheme with " << num_imports << " imports, "
       << num_exports << " exports, " << num_diagnostics << " diagnostics\"\n";
    ss << "  language: cpp\n";
    ...
    std::string config_file = "test_combination_" + std::to_string(num_imports) +
                              "_" + std::to_string(num_exports) + "_" +
    ...
    EXPECT_TRUE(RunSchemeGenerator(config_file))
        << "Generator failed for combination: " << num_imports << " imports, "
        << num_exports << " exports, " << num_diagnostics << " diagnostics";
