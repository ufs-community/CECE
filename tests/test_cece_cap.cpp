...
static std::string g_config_file = "cece_config.yaml";

TEST(CECECapTest, Lifecycle) {
    std::cout << "Test: Creating config from: " << g_config_file << std::endl;

    // Only create default config if no custom config specified
    if (g_config_file == "cece_config.yaml") {
        std::ofstream config_file("cece_config.yaml");
        config_file << "species:\n  nox: []\nphysics_schemes: []\n";
        config_file.close();
    }

    // Set the config file path for CECE
    cece_set_config_file_path(g_config_file.c_str(), static_cast<int>(g_config_file.length()));

    int rc = -1;
    void* data_ptr = nullptr;

    std::cout << "Test: Calling cece_core_initialize_p1" << std::endl;
    cece_core_initialize_p1(&data_ptr, &rc);
    EXPECT_EQ(rc, 0);
    EXPECT_NE(data_ptr, nullptr);

    std::cout << "Test: Calling cece_core_finalize" << std::endl;
