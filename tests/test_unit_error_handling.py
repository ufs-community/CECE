...
class TestStateManagement:
    ...
    def test_state_creation_with_valid_dimensions(self):
        ...
        state = cece.CeceState(nx=10, ny=20, nz=30)
    ...
    def test_state_creation_with_large_dimensions(self):
        ...
        state = cece.CeceState(nx=1000, ny=1000, nz=100)
    ...
    def test_state_creation_with_small_dimensions(self):
        ...
        state = cece.CeceState(nx=1, ny=1, nz=1)
    ...
    def test_state_creation_with_negative_nz_raises_error(self):
        ...
        with pytest.raises(ValueError):
            cece.CeceState(nx=10, ny=10, nz=-1)
...
class TestErrorHandling:
    ...
    def test_exception_classes_inherit_from_base(self):
        ...
        assert issubclass(cece.CeceConfigError, cece.CeceException)
        assert issubclass(cece.CeceComputationError, cece.CeceException)
        assert issubclass(cece.CeceStateError, cece.CeceException)

    def test_get_last_error_returns_string_or_none(self):
        """Test that get_last_error returns string or None."""
        error = cece.get_last_error()
...
class TestTemporalCycles:
    ...
    def test_compute_with_temporal_parameters(self):
        ...
        try:
            cece.compute(state, config, hour=12, day_of_week=3, month=6)
        except RuntimeError:
...
class TestEdgeCases:
    ...
    def test_state_with_very_large_dimensions(self):
        ...
        state = cece.CeceState(nx=100, ny=100, nz=100)
