...
AcesStandaloneWriter::~AcesStandaloneWriter() { Finalize(); }

int AcesStandaloneWriter::Initialize(const std::string& start_time_iso8601,
...
int AcesStandaloneWriter::WriteTimeStep(ESMC_State export_state,
  ...
  if (config_.fields.empty()) {
      std::cerr << "WARNING: No fields specified in output config and ESMC_State cannot be iterated reliably in C API. Skipping output." << std::endl;
      return 0;
  }
  ...
  for (const auto& name : config_.fields) {
      ...
      if (rc == ESMF_SUCCESS) {
          ...
          if (rc != ESMF_SUCCESS) {
              std::cerr << "ERROR: ESMC_FieldWrite failed for " << name << " to " << filepath << " rc=" << rc << std::endl;
          } else {
              first_field = false;
          }
      } else {
...
int AcesStandaloneWriter::WriteTimeStep(
    ...
    for (const auto& [name, view] : export_fields) {
        ...
        int rc_field;
        // ESMC_FieldCreateGridTypeKind(grid, typekind, staggerloc, gridToFieldMap, ungriddedLBound, ungriddedUBound, name, rc)
        ESMC_Field field = ESMC_FieldCreateGridTypeKind(grid, ESMC_TYPEKIND_R8,
...
std::string AcesStandaloneWriter::ResolveFilename(double time_seconds_since_start) const {
  ...
  char buffer[16];
  strftime(buffer, sizeof(buffer), "%Y", tm_current); replace_all(result, "{YYYY}", buffer);
  strftime(buffer, sizeof(buffer), "%m", tm_current); replace_all(result, "{MM}", buffer);
  strftime(buffer, sizeof(buffer), "%d", tm_current); replace_all(result, "{DD}", buffer);
  strftime(buffer, sizeof(buffer), "%H", tm_current); replace_all(result, "{HH}", buffer);
  strftime(buffer, sizeof(buffer), "%M", tm_current); replace_all(result, "{mm}", buffer);
  strftime(buffer, sizeof(buffer), "%S", tm_current); replace_all(result, "{ss}", buffer);
  return result;
}
