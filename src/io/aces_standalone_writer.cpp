...
int AcesStandaloneWriter::InitializeWithCoords(const std::string& start_time_iso8601, int nx,
                                               int ny, int nz,
...
int AcesStandaloneWriter::WriteTimeStep(const std::unordered_map<std::string, DualView3D>& fields,
    ...
    ACES_LOG_INFO("[ACES] Writing time step " + std::to_string(step) +
    ...
    try {
        ...
        check_nc(nc_put_att_text(ncid, NC_GLOBAL, "Conventions", 6, "CF-1.8"),
                 "global Conventions");
        check_nc(nc_put_att_text(ncid, NC_GLOBAL, "title", 28, "ACES Atmospheric Emissions"),
                 "global title");
        ...
        for (size_t i = 0; i < field_names.size(); ++i) {
            ...
            if (h_view.size() != total_elements) {
                ACES_LOG_ERROR("Size mismatch in field '" + name +
            ...
            for (int k = 0; k < nz_; k++) {          // level
                for (int j = 0; j < ny_; j++) {      // latitude index
                    for (int i = 0; i < nx_; i++) {  // longitude index
                        // Correct indexing: ACES uses (lon, lat, lev) order - i=longitude,
                        // j=latitude
                        size_t kokkos_idx =
                            i + j * nx_ +
                        ...
                        if (kokkos_idx >= total_elements || netcdf_idx >= total_elements) {
                            ACES_LOG_ERROR("Index out of bounds in field '" + name +
