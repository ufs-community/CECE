program test_tide_bilinear
  ! Diagnostic test: bilinear regridding from a 2×2 lat/lon source grid to a
  ! 4-element model mesh whose element centres coincide exactly with the
  ! source cell centres.
  !
  ! When source and destination are geometrically identical, bilinear
  ...
  !
  ! The test also prints the routehandle statistics so the weights can be
  ! inspected in the output if the result is unexpected.
  !
  ! Grid layout (same as test_tide_passthrough_2d):
  ...
  character(len=256)     :: config_file = "test_bilinear.yaml"
  character(len=256)     :: data_file = "test_bilinear_data.nc"
  ...
  ! Same distinct values as the passthrough_2d test.
  ...
  ! 1% relative tolerance — bilinear on same grid should be exact,
  ...
  if (my_task == 0) then
    ...
    write(99, '(a)') 'streams:'
    write(99, '(a)') '  - name: bilinear_test_stream'
    write(99, '(a)') '    tax_mode: "cycle"'
    write(99, '(a)') '    time_interp: "linear"'
    write(99, '(a)') '    map_algo: "bilinear"'
  ...
  ! -----------------------------------------------------------------------
  ! Build a 4-element (2×2) model mesh whose element CENTRES coincide with
  ! the source cell centres (lon=0.25/0.75, lat=0.25/0.75).
  !
  ! To achieve element centres at (0.25, 0.25), (0.75, 0.25), (0.25, 0.75),
  ! (0.75, 0.75), the node corners must be at the half-way points 0, 0.5, 1.
  !
  ! Node grid:
  !   7:(0,1)   8:(0.5,1)  9:(1,1)
  ...
  if (my_task == 0) then
    ...
    ! elementCoords: ESMF 8.x requires element centre coordinates when the mesh
    ! is used as a bilinear (or other) regrid destination.  Without them,
    ! ESMF_MeshGet(ownedElemCoords=...) fails inside ESMF_FieldRegridStore.
  ...
  if (rc /= ESMF_SUCCESS) then
    print *, "TIDE bilinear test: tide_init failed"
    stop 1
  end if
  ...
  if (rc /= ESMF_SUCCESS) then
    print *, "TIDE bilinear test: tide_advance failed"
    stop 1
  end if

  call tide_get_ptr(tide, "flux", data_ptr, rc=rc)
  if (rc /= ESMF_SUCCESS) then
    print *, "TIDE bilinear test: tide_get_ptr failed"
    stop 1
  end if
  ...
  if (my_task == 0) then
    ...
    if (all_passed) then
      print *, "TIDE bilinear test: all 4 values within tolerance. PASSED."
    else
      print *, "TIDE bilinear test: one or more values out of tolerance. FAILED."
      ! Don't stop 1 here — let us see all values first.
      stop 1
    end if
  end if
...
end program test_tide_bilinear
