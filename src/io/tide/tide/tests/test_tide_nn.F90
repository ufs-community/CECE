program test_tide_nn
  ! Diagnostic test: nearest-source-to-destination (nn) regridding from a
  ! 2×2 lat/lon source grid to a 4-element model mesh whose element centres
  ! coincide exactly with the source cell centres.
  !
  ! When source and destination centres match exactly, nn regridding must
  ! reproduce the source values to machine precision — each destination
  ! element maps to the unique nearest source cell.
  !
  ! nn only requires ESMF_STAGGERLOC_CENTER on the source grid (no corner
  ! staggerloc / no lon_bnds/lat_bnds needed), so the source NetCDF has
  ! plain 1-D lat/lon without bounds variables.
  !
  ...
  character(len=256)     :: config_file = "test_nn.yaml"
  character(len=256)     :: data_file = "test_nn_data.nc"
  ...
  ! Same distinct values as the bilinear/consd tests.
  ...
  ! 1% relative tolerance — nn on same grid should be exact.
  ...
  ! -----------------------------------------------------------------------
  ! Write 2×2 NetCDF (no bounds vars — nn only needs cell centres) and YAML.
  ! -----------------------------------------------------------------------
  if (my_task == 0) then
    block
      ...
      cf_rc = nf90_put_var(ncid, varid, [0.25d0, 0.75d0])  ! cell centres
  ...
  if (rc /= ESMF_SUCCESS) then
    print *, "TIDE nn test: tide_init failed"
    stop 1
  end if
  ...
  if (rc /= ESMF_SUCCESS) then
    print *, "TIDE nn test: tide_advance failed"
    stop 1
  end if

  call tide_get_ptr(tide, "flux", data_ptr, rc=rc)
  if (rc /= ESMF_SUCCESS) then
    print *, "TIDE nn test: tide_get_ptr failed"
    stop 1
  end if
  ...
  if (my_task == 0) then
    ...
    if (all_passed) then
      print *, "TIDE nn test: all 4 values within tolerance. PASSED."
    else
      print *, "TIDE nn test: one or more values out of tolerance. FAILED."
      stop 1
    end if
  end if
