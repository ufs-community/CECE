...
contains
  ...
  subroutine cece_sea_salt_run(horizontal_loop_extent, vertical_layer_dimension, &
    ...
    real(kind=8), intent(out) :: sea_salt_emission_flux_fine(:,:)
    real(kind=8), intent(out) :: sea_salt_emission_flux_coarse(:,:)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
  ...
  subroutine cece_sea_salt_finalize(errmsg, errflg)
    ...
    if (g_init_count <= 0) then
      ...
      g_initialized = .false.
      g_init_count = 0
      if (rc /= 0) then; errflg = 1; errmsg = 'sea_salt_finalize: core finalize failed'; return; end if
    end if
  end subroutine

end module cece_ccpp_sea_salt
