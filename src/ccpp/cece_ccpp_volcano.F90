module cece_ccpp_volcano
  ...
  use cece_ccpp_state, only: g_cece_data_ptr, g_init_count, g_initialized, &
    cece_ccpp_core_finalize, cece_ccpp_core_init, cece_ccpp_get_export_field, cece_ccpp_scheme_finalize, cece_ccpp_scheme_init, cece_ccpp_scheme_run, cece_ccpp_sync_export_to_host, cece_ccpp_sync_import_to_device
  implicit none
  private
  public :: cece_ccpp_volcano_init, cece_ccpp_volcano_run, cece_ccpp_volcano_finalize, &
            cece_ccpp_volcano_timestep_init, cece_ccpp_volcano_timestep_finalize


contains

  subroutine cece_ccpp_volcano_init(horizontal_loop_extent, vertical_layer_dimension, &
    ...
    if (.not. g_initialized) then
      ...
      if (rc /= 0) then
        errflg = 1
        errmsg = 'cece_ccpp_volcano_init: core init failed'
        return
      end if
    ...
    if (rc /= 0) then
      errflg = 1
      errmsg = 'cece_ccpp_volcano_init: scheme init failed'
      return
    end if
    g_init_count = g_init_count + 1
  end subroutine

  subroutine cece_ccpp_volcano_run(horizontal_loop_extent, vertical_layer_dimension, &
  ...
  subroutine cece_ccpp_volcano_timestep_init(errmsg, errflg)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
    errmsg = ''
  ...
  subroutine cece_ccpp_volcano_timestep_finalize(errmsg, errflg)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
    errmsg = ''
  ...
  subroutine cece_ccpp_volcano_finalize(errmsg, errflg)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
    errmsg = ''
    ...
    if (.not. g_initialized) then
      errflg = 1
      errmsg = 'cece_ccpp_volcano_finalize: not initialized'
      return
    end if
