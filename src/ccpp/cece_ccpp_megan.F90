module cece_ccpp_megan
  ...
  implicit none
  private
  public :: cece_megan_init, cece_megan_run, cece_megan_finalize, &
            cece_megan_timestep_init, cece_megan_timestep_finalize
...
contains
  ...
  subroutine cece_megan_init(horizontal_loop_extent, vertical_layer_dimension, &
    ...
    if (.not. g_initialized) then
      ...
      if (rc /= 0) then
        errflg = 1
        errmsg = 'cece_megan_init: core init failed'
        return
      end if
    ...
    if (rc /= 0) then
      errflg = 1
      errmsg = 'cece_megan_init: scheme init failed'
      return
    end if
    g_init_count = g_init_count + 1
  end subroutine

  subroutine cece_megan_run(horizontal_loop_extent, vertical_layer_dimension, &
  ...
  subroutine cece_megan_timestep_init(errmsg, errflg)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
    errmsg = ''
  ...
  subroutine cece_megan_timestep_finalize(errmsg, errflg)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
    errmsg = ''
  ...
  subroutine cece_megan_finalize(errmsg, errflg)
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer(c_int) :: rc
    errmsg = ''
    ...
    if (.not. g_initialized) then
      errflg = 1
      errmsg = 'cece_megan_finalize: not initialized'
      return
    end if
