/**
 * @file aces_core_initialize_p2_wrapper.cpp
 * @brief Wrapper for ACES Phase 2 initialization that accepts ESMF objects.
 *
...
void aces_core_initialize_p2(void* data_ptr, int* nx, int* ny, int* nz, int* rc) {
    aces_core_initialize_p2_impl(data_ptr, nx, ny, nz, rc);
}
...
void aces_core_initialize_p2_esmf(void* data_ptr, void* gcomp_ptr, void* importState_ptr,
    ...
    std::cout << "INFO: ACES Initialize Phase 2 (ESMF wrapper) - Extracting grid dimensions"
              << std::endl;

    // Validate that Phase 1 completed successfully
    if (data_ptr == nullptr) {
        std::cerr << "ERROR in aces_core_initialize_p2_esmf: Internal data pointer is null - "
    ...
    if (grid_ptr == nullptr) {
        std::cerr << "ERROR in aces_core_initialize_p2_esmf: Grid pointer is null" << std::endl;
        if (rc != nullptr) {
            *rc = -1;
        }
        return;
