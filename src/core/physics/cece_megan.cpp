...
void MeganScheme::Initialize(const conf::Value& config, CeceDiagnosticManager* diag_manager) {
    ...
    if (megan_method_ != "native" && megan_method_ != "hemco_3_12_1") {
        CECE_LOG_WARNING("MeganScheme: unknown megan_method '" + megan_method_ + "'; falling back to 'native'");
    ...
    if (megan_method_ == "hemco_3_12_1") {
        ...
        hemco_par_avg_umol_ = config["hemco_par_avg_umol"].double_or(hemco_megan::v3_12_1::kParAvgUmol);
        hemco_t_avg_15_k_ = config["hemco_t_avg_15_k"].double_or(hemco_megan::v3_12_1::kTAvg15);
...
void MeganScheme::Run(CeceImportState& import_state, CeceExportState& export_state) {
    ...
    if (megan_method_ == "hemco_3_12_1") {
        ...
        Kokkos::parallel_for(
            "MeganKernel_HEMCO3121", Kokkos::MDRangePolicy<Kokkos::DefaultExecutionSpace, Kokkos::Rank<2>>({0, 0}, {nx, ny}),
