...
double ginoux_marticorena_threshold(double diameter, double soil_density,
    ...
    return 0.13 * Kokkos::sqrt(soil_density * grav * diameter / air_dens)
               * Kokkos::sqrt(1.0 + 6.0e-7 / (soil_density * grav
                              * Kokkos::pow(diameter, 2.5)))
               / Kokkos::sqrt(1.928 * Kokkos::pow(reynol, 0.092) - 1.0);
}
...
double ginoux_moisture_threshold(double u_thresh0, double gwettop) {
    ...
    return Kokkos::max(0.0, u_thresh0 * (1.2 + 0.2
           * Kokkos::log10(Kokkos::max(1.0e-3, gwettop))));
}
...
void GinouxScheme::Initialize(const YAML::Node& config, CeceDiagnosticManager* diag_manager) {
    ...
    if (config["particle_radii"]) {
        ...
        particle_radii_ = Kokkos::View<double*, Kokkos::DefaultExecutionSpace>(
            "particle_radii", num_bins_);
    ...
    std::cout << "GinouxScheme: Initialized. ch_du=" << ch_du_ << " grav=" << grav_
              << " num_bins=" << num_bins_ << "\n";
}
...
void GinouxScheme::Run(CeceImportState& import_state, CeceExportState& export_state) {
    ...
    if (u10m.data() == nullptr || v10m.data() == nullptr || gwettop.data() == nullptr ||
        oro.data() == nullptr || fraclake.data() == nullptr || du_src.data() == nullptr ||
        radius.data() == nullptr || emissions.data() == nullptr) {
        return;
    }
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(int i, int j) {
            ...
            double w10m = Kokkos::sqrt(u10m(i, j, 0) * u10m(i, j, 0) +
                                       v10m(i, j, 0) * v10m(i, j, 0));
            ...
            for (int n = 0; n < nbins; ++n) {
                ...
                double u_thresh0 = ginoux_marticorena_threshold(
                    diameter, soil_density, grav, air_dens);
                ...
                if (w10m > u_thresh) {
                    emissions(i, j, n) = (1.0 - fraclake_val) * w10m * w10m *
                                         (w10m - u_thresh);
