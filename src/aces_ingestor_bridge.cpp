 ...
 *
 * @param data_ptr Pointer to the C++ AcesInternalData structure (opaque).
 ...
 */
void aces_ingestor_set_field(void* data_ptr, const char* field_name, int name_len,
                             const double* field_data, int n_lev, int n_elem, int* rc) {
    if (!data_ptr) {
        ACES_LOG_ERROR("[ACES] aces_ingestor_set_field: Null data pointer");
        if (rc) *rc = -1;
        return;
    }

    if (!field_data) {
        ACES_LOG_ERROR("[ACES] aces_ingestor_set_field: Null field data pointer");
        if (rc) *rc = -1;
        return;
    }

    if (!field_name || name_len <= 0) {
        ACES_LOG_ERROR("[ACES] aces_ingestor_set_field: Invalid field name");
        if (rc) *rc = -1;
        return;
    }
