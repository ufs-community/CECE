#!/usr/bin/env python3
"""
hemco_to_cece.py - Convert HEMCO_Config.rc to CECE YAML configuration.
...
def _infer_vdist(be_entry):
    ...
    Returns:
        dict or None: Dict suitable for the CECE layer 'vdist' key, or None for surface.
    """
...
def convert_hemco_to_cece(hemco_config_path, output_path, diagn_path=None):
    ...
    Args:
        ...
        output_path (str): Path for the output CECE YAML file
    ...
    for sf_id, sf in parser.scale_factors.items():
        ...
        if "met" in sf_key or sf["file"] == "-":
            ...
            cece_config["scale_factors"].pop(sf_key, None)
    ...
    if diagnostics:
        cece_config["diagnostics"] = diagnostics

    with open(output_path, "w") as f:
        yaml.dump(cece_config, f, sort_keys=False, default_flow_style=False)


def convert_hemco_diagn(diagn_path):
    """
    Convert a HEMCO_Diagn.rc file to an CECE diagnostics configuration dict.

    Maps HEMCO diagnostic specifications to CECE YAML format, preserving
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "-o",
        "--output",
        default="cece_config.yaml",
        help="Path to the output CECE configuration file (default: cece_config.yaml)",
    )
    arg_parser.add_argument(
        "--diagn",
        default=None,
    ...
    convert_hemco_to_cece(args.hemco_config, args.output, diagn_path=args.diagn)
