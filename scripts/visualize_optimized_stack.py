...
def visualize_optimized_stack(config_path):
    ...
    for species, layers in species_dict.items():
        ...
        hierarchies = [l.get('hierarchy', 0) for l in sorted_layers]
        labels = [f"{l.get('field', '??')}\n({l.get('operation', 'add')})" for l in sorted_layers]

        plt.figure(figsize=(10, 6))
        colors = ['green' if l.get('operation') == 'add' else 'orange' for l in sorted_layers]

        plt.bar(range(len(hierarchies)), hierarchies, color=colors, alpha=0.7, edgecolor='black')

        # Add a "Fused Kernel" bracket/text
        plt.text(len(hierarchies)/2 - 0.5, max(hierarchies) * 1.1 if hierarchies else 1,
                 "FUSED KOKKOS KERNEL", ha='center', fontweight='bold', color='red',
